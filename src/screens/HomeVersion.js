  ...
  const StoryCircle = ({ label }) => (
    <View style={styles.storyContainer}>
      <View style={styles.storyBorder}>
        <Image source={{ uri: 'https://via.placeholder.com/100' }} style={styles.storyImage} />
      </View>
        ...
        <FlatList
          ...
          renderItem={({ item: short }) => (
            <View style={styles.shortCard}>
              ...
              <TouchableOpacity style={styles.moreIconShort} onPress={() => setOptionsVisible(true)}>
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ paddingHorizontal: SPACING.lg }}>
          {CONTINUE_WATCHING_DATA.map((c) => (
            <View key={c.id} style={styles.continueCard}>
              ...
              <View style={styles.playButtonSmall}><Icon name="play" size={16} color="#fff" /></View>
      ...
      <View style={styles.videoCard}>
        ...
        <View style={styles.videoDetails}>
          ...
          <View style={styles.videoInfo}>
            ...
            <Text style={styles.videoMetaMerged}>{item.author} • {item.views} • {item.time}</Text>
          </View>
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.navBar}>
        <Text style={styles.logoText}>eat<Text style={{ color: COLORS.primaryOrange }}>ix</Text></Text>
        <View style={styles.navIcons}>
          <Icon name="magnify" size={26} color={COLORS.textPrimary} style={styles.iconSpaced} />
          <Icon name="bell-outline" size={26} color={COLORS.textPrimary} style={styles.iconSpaced} />
        ...
        ListHeaderComponent={
          <>
            ...
            <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.storyScroll}>
              {STORIES.map(name => <StoryCircle key={name} label={name} />)}
            </ScrollView>
          </>
        }
      />
      ...
      <View style={styles.tabBar}>
        ...
        <Icon name="play-box-multiple-outline" size={28} color={COLORS.gray500} />
        <View style={styles.fabContainer}><Icon name="plus" size={30} color={COLORS.white} /></View>
        <Icon name="youtube-subscription" size={28} color={COLORS.gray500} />
        <Icon name="library-outline" size={28} color={COLORS.gray500} />
      </View>

      {/* --- Modals --- */}
      <Modal animationType="slide" transparent visible={optionsVisible} onRequestClose={() => setOptionsVisible(false)}>
        <Pressable style={styles.modalOverlay} onPress={() => setOptionsVisible(false)}>
          <View style={styles.modalContent}>
            ...
            <TouchableOpacity style={styles.optionRow} onPress={openReportModal}><Icon name="alert-circle-outline" size={24} color={COLORS.primaryOrange} /><Text style={[styles.optionText, { color: COLORS.primaryOrange }]}>Report</Text></TouchableOpacity>
          </View>
        </Pressable>
      </Modal>
        ...
        <Pressable style={styles.modalOverlay} onPress={() => setReportVisible(false)}>
          <View style={styles.modalContent}>
            ...
            {REPORT_REASONS.map((reason) => (
              <TouchableOpacity key={reason} activeOpacity={0.8} style={styles.reportOptionRow} onPress={() => setSelectedReason(reason)}>
                <Icon name={selectedReason === reason ? "radiobox-marked" : "radiobox-blank"} size={24} color={COLORS.primaryOrange} />
                <Text style={styles.reportOptionText}>{reason}</Text>
              </TouchableOpacity>
            ))}
            <View style={styles.reportActionRow}>
              <TouchableOpacity style={styles.cancelButton} onPress={() => setReportVisible(false)}><Text style={styles.cancelButtonText}>Cancel</Text></TouchableOpacity>
              <TouchableOpacity style={styles.submitButton} onPress={() => setReportVisible(false)}><Text style={styles.submitButtonText}>Report</Text></TouchableOpacity>
            </View>
          </View>
        </Pressable>
      </Modal>
...
const styles = StyleSheet.create({
  ...
  navBar: { flexDirection: 'row', justifyContent: 'space-between', paddingHorizontal: SPACING.lg, alignItems: 'center' },
  ...
  profileMini: { width: 32, height: 32, borderRadius: 16, backgroundColor: COLORS.gray300 },
  chipScroll: { marginVertical: SPACING.md, paddingLeft: SPACING.lg },
  chip: { paddingHorizontal: SPACING.lg, paddingVertical: SPACING.sm, borderRadius: 25, borderWidth: 1, borderColor: COLORS.primaryOrange, marginRight: SPACING.md },
  ...
  storyBorder: { padding: 3, borderRadius: 40, borderWidth: 2, borderColor: COLORS.primaryOrange },
  storyImage: { width: 65, height: 65, borderRadius: 32.5 },
  storyLabel: { fontSize: 11, marginTop: 5, color: '#666' },
  feedHeaderRow: { flexDirection: 'row', alignItems: 'center', paddingHorizontal: SPACING.lg, marginVertical: 12 },
  ...
  playButtonSmall: { position: 'absolute', bottom: 8, left: 8, backgroundColor: COLORS.primaryOrange, borderRadius: 4, padding: 2 },
  progressBar: { position: 'absolute', bottom: 0, left: 0, height: 3, width: '70%', backgroundColor: COLORS.primaryOrange },
  ...
  channelIcon: { width: 40, height: 40, borderRadius: 20, backgroundColor: COLORS.gray200 },
  videoInfo: { flex: 1, marginHorizontal: 12 },
  videoTitleMerged: { fontSize: 15, fontWeight: '600', color: '#000', lineHeight: 20 },
  videoMetaMerged: { fontSize: 12, color: '#606060', marginTop: 2 },
  tabBar: { height: 70, flexDirection: 'row', justifyContent: 'space-around', alignItems: 'center', borderTopWidth: 1, borderColor: COLORS.gray200, backgroundColor: '#fff' },
  fabContainer: { width: 50, height: 50, borderRadius: 25, backgroundColor: COLORS.primaryOrange, justifyContent: 'center', alignItems: 'center', marginTop: -20 },
  ...
  cancelButtonText: { color: COLORS.primaryOrange, fontWeight: 'bold', fontSize: 16 },
  submitButton: { flex: 1, backgroundColor: COLORS.primaryOrange, paddingVertical: 15, borderRadius: 30, marginLeft: 10, alignItems: 'center' },
