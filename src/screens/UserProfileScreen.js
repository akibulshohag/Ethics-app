...
const PROMO_CHIPS = ['Tandoori Planet', 'Steely Balky', 'Bangla Food', 'UK Grill'];
...
const DUMMY_VIDEOS = [
  {
    ...
    duration: '16:27',
    thumbnail: 'https://images.unsplash.com/photo-1604908176997-125f25cc6f3d?q=80&w=400',
    channel: 'BBC Earth',
    views: '9.5M views',
    time: '5 months ago',
  ...
  },
  {
    ...
    duration: '56:44',
    thumbnail: 'https://images.unsplash.com/photo-1593696140826-c58bed479fcb?q=80&w=400',
    ...
    <View style={styles.aboutContainer}>
      ...
      <View style={styles.aboutSection}>
        ...
        <View style={styles.infoRow}>
          <MaterialCommunityIcons name="map-marker-outline" size={22} color="#757575" />
          <Text style={styles.infoText}>United kingdom</Text>
        </View>
        <View style={styles.infoRow}>
          <MaterialCommunityIcons name="information-outline" size={22} color="#757575" />
          <Text style={styles.infoText}>Joined December 20, 2026</Text>
        </View>
        <View style={styles.infoRow}>
          <MaterialCommunityIcons name="trending-up" size={22} color="#757575" />
    ...
    <ScrollView style={styles.detailContainer} showsVerticalScrollIndicator={false}>
      <View style={styles.promoCard}>
        <View style={styles.promoCardHeader}>
          ...
          <TouchableOpacity style={styles.orderBtn}><Text style={styles.orderBtnText}>Order Now</Text></TouchableOpacity>
        </View>
        <Image source={{ uri: selectedPromo.thumbnail }} style={styles.promoDetailImage} />
        ...
        {[1, 2, 3].map((_, i) => (
          <View key={i} style={styles.checkRow}>
            <MaterialCommunityIcons name="check-circle-outline" size={16} color="#757575" />
        ...
        {[...DUMMY_VIDEOS, ...DUMMY_VIDEOS].map((item, index) => (
          <View key={index} style={styles.gridItem}>
            ...
            <View style={styles.gridMetaRow}>
              ...
              <MaterialCommunityIcons name="dots-vertical" size={18} color="#757575" />
            </View>
            <Text style={styles.promoHighlight}>Get Flat <Text style={{ color: '#FF7F00' }}>30% Off</Text></Text>
          </View>
        ))}
      </View>
    </ScrollView>
  ...
  const renderHeader = () => (
    <View style={styles.bgWhite}>
      <View style={styles.topNav}>
        <TouchableOpacity onPress={() => selectedPromo ? setSelectedPromo(null) : null}>
          <Ionicons name="arrow-back" size={24} color="#000" />
        </TouchableOpacity>
        <View style={styles.topNavRight}>
          <TouchableOpacity style={{ marginRight: 20 }}><Ionicons name="search-outline" size={24} color="#000" /></TouchableOpacity>
          <TouchableOpacity><MaterialCommunityIcons name="comment-text-outline" size={24} color="#000" /></TouchableOpacity>
        </View>
      </View>
      ...
      {!selectedPromo && (
        <>
          <View style={styles.tabBar}>
            {TABS.map(tab => (
              <TouchableOpacity key={tab} onPress={() => setActiveTab(tab)} style={[styles.tabButton, activeTab === tab && styles.activeTabBorder]}>
          ...
          {activeTab === 'Profile' && (
            <View style={styles.profileHeader}>
              ...
              <View style={styles.nameRow}>
                ...
                <MaterialCommunityIcons name="check-circle" size={18} color="#4A89F3" />
              </View>
              ...
              <TouchableOpacity style={styles.moreChannelRow}>
                ...
                <MaterialCommunityIcons name="chevron-right" size={16} color="#757575" />
              </TouchableOpacity>
                ...
                <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ paddingLeft: 16 }}>
                  {DUMMY_VIDEOS.map((item, index) => (
                    <View key={index} style={styles.horizontalCard}>
                      <View>
                        <Image source={{ uri: item.thumbnail }} style={styles.horizontalThumb} />
                      ...
                      <Text style={styles.horizontalChannel}>World of Music</Text>
                    </View>
                  ))}
                </ScrollView>
              </View>
          ...
          {activeTab === 'Promotions' && (
            <View style={styles.promoTabTitleArea}>
              <View style={styles.promoMainRow}>
                ...
                <MaterialCommunityIcons name="check-circle" size={20} color="#4A89F3" />
              </View>
            </View>
          )}
            ...
            <>
              ...
              <View style={styles.chipsContainer}>
                {PROMO_CHIPS.map((chip, i) => (
                  <TouchableOpacity key={i} style={[styles.chip, i === 0 && activeTab === 'Promotions' && styles.activeChip]}>
                    <Text style={[styles.chipText, i === 0 && activeTab === 'Promotions' && styles.activeChipText]}>{chip}</Text>
                  </TouchableOpacity>
                ))}
              </View>
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      {selectedPromo ? (
        <>
          <View style={styles.topNav}>
            <TouchableOpacity onPress={() => setSelectedPromo(null)}><Ionicons name="arrow-back" size={24} color="#000" /></TouchableOpacity>
          </View>
        ...
        <FlatList
          ...
          data={activeTab === 'Profile' ? DUMMY_VIDEOS : [...DUMMY_VIDEOS, ...DUMMY_VIDEOS]}
          keyExtractor={(item, index) => `${activeTab}-${item.id}-${index}`}
          key={activeTab}
          numColumns={activeTab === 'Promotions' ? 2 : 1}
          columnWrapperStyle={activeTab === 'Promotions' ? { paddingHorizontal: 16, justifyContent: 'space-between' } : null}
          renderItem={({ item }) => (
            <TouchableOpacity
              onPress={() => activeTab === 'Promotions' && setSelectedPromo(item)}
              style={activeTab === 'Promotions' ? styles.gridItem : styles.fullVideoItem}
            >
              <View>
                <Image source={{ uri: item.thumbnail }} style={activeTab === 'Promotions' ? styles.gridThumb : styles.fullThumb} />
                <View style={styles.durationBadge}><Text style={styles.durationText}>{item.duration}</Text></View>
              </View>
              <View style={styles.fullVideoInfo}>
                <View style={styles.channelAvatarSmall} />
                <View style={{ flex: 1 }}>
                  <Text style={styles.fullVideoTitle} numberOfLines={2}>{item.title}</Text>
                  <Text style={styles.fullVideoMeta}>{item.channel}  •  {item.views}</Text>
                </View>
...
const styles = StyleSheet.create({
  ...
  featuredDesc: { fontSize: 13, color: '#616161', lineHeight: 18, marginVertical: 10 },
  learnMoreBtn: { flexDirection: 'row', alignItems: 'center', alignSelf: 'flex-start', borderWidth: 1, borderColor: '#ccc', paddingHorizontal: 15, paddingVertical: 6, borderRadius: 20 },
  ...
  horizontalMeta: { flexDirection: 'row', justifyContent: 'space-between', marginTop: 8 },
  ...
  aboutHeaderRow: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 },
  ...
  chipsContainer: { flexDirection: 'row', flexWrap: 'wrap', paddingHorizontal: 16, marginBottom: 15 },
  chip: { borderWidth: 1, borderColor: '#FF7F00', borderRadius: 20, paddingHorizontal: 15, paddingVertical: 6, marginRight: 8, marginBottom: 8 },
  activeChip: { backgroundColor: '#FF7F00' },
  chipText: { color: '#FF7F00', fontWeight: '500' },
  activeChipText: { color: '#fff' },

  sectionHeaderRow: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingHorizontal: 16, marginVertical: 10 },
  sectionTitle: { fontSize: 18, fontWeight: 'bold' },
  durationBadge: { position: 'absolute', bottom: 6, right: 6, backgroundColor: 'rgba(0,0,0,0.8)', paddingHorizontal: 4, borderRadius: 4 },
  ...
  channelAvatarSmall: { width: 36, height: 36, borderRadius: 18, backgroundColor: '#333', marginRight: 12 },
  ...
  gridMetaRow: { flexDirection: 'row', justifyContent: 'space-between', marginTop: 8 },
  gridItemTitle: { fontSize: 13, fontWeight: '600', flex: 1 },
  promoHighlight: { fontSize: 12, fontWeight: 'bold', marginTop: 4, color: '#757575' },

  detailContainer: { flex: 1, backgroundColor: '#fff' },
  promoCard: { margin: 16, padding: 16, backgroundColor: '#FFF8F2', borderRadius: 16, borderWidth: 1, borderColor: '#FFE8D1' },
  promoCardHeader: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 15 },
  promoCardTitle: { fontSize: 18, fontWeight: 'bold' },
  orderBtn: { backgroundColor: '#FF7F00', paddingHorizontal: 15, paddingVertical: 6, borderRadius: 20 },
  orderBtnText: { color: '#fff', fontWeight: 'bold', fontSize: 12 },
  promoDetailImage: { width: '100%', height: 160, borderRadius: 12, marginBottom: 15 },
  promoCodeLabel: { fontSize: 15, fontWeight: 'bold', marginBottom: 10 },
  promoCodeBox: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', borderStyle: 'dashed', borderWidth: 1, borderColor: '#FF7F00', padding: 10, borderRadius: 10, backgroundColor: '#fff', marginBottom: 15 },
  promoCodeText: { fontSize: 16, fontWeight: 'bold' },
  copyBtn: { backgroundColor: '#FF7F00', paddingHorizontal: 18, paddingVertical: 6, borderRadius: 20 },
